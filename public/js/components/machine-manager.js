...
class MachineManager {
    ...
    buildTileHeader(machine) {
        ...
        actions.appendChild(this.createIconButton({
            ...
            onClick: () => this.showModal(machine.id)
        }));
        ...
        if (machine.status === 'ready') {
            actions.appendChild(this.createIconButton({
                ...
                onClick: () => this.showConsoleModal(machine.id)
            }));
        }
        ...
        actions.appendChild(this.createIconButton({
            ...
            onClick: () => this.removeMachine(machine.id)
        }));
    ...
    buildTileDetails(machine) {
        ...
        activityRow.append(
            ...
            this.createElement(
                ...
                machine.lastSeen ? this.formatTime(machine.lastSeen) : '—'
            )
        );
    ...
    buildTileFooter(machine) {
        ...
        if (machine.status === 'ready') {
            ...
            const disconnectBtn = this.createFooterButton('Déconnecter', 'secondary', () => this.disconnectMachine(machine.id));
        ...
        if (machine.status === 'disconnected') {
            const action = machine.needsAuthorization ? () => this.authorizeAndConnect(machine.id) : () => this.connectExistingMachine(machine.id);
            const connectBtn = this.createFooterButton(machine.needsAuthorization ? 'Autoriser le port' : 'Connecter', 'primary', action);
        ...
        if (machine.status === 'error') {
            const action = machine.lastError === 'NotAllowedError'
                ? () => this.authorizeAndConnect(machine.id)
                : () => this.connectExistingMachine(machine.id);
            const retryBtn = this.createFooterButton(machine.lastError === 'NotAllowedError' ? 'Autoriser le port' : 'Réessayer', 'primary', action);
