...
import { PrinterModelV2, db } from '../models/printer.model.v2.js';
...
router.put('/printers/:id/config', asyncHandler(async (req, res) => {
    const printer = await PrinterModelV2.findById(req.params.id);
    if (!printer) {
        return res.status(404).json({ success: false, error: 'Printer not found' });
    }
...
router.put('/printers/:id/hardware', asyncHandler(async (req, res) => {
    const printer = await PrinterModelV2.findById(req.params.id);
    if (!printer) {
        return res.status(404).json({ success: false, error: 'Printer not found' });
    }
...
router.put('/printers/:id/firmware', asyncHandler(async (req, res) => {
    const printer = await PrinterModelV2.findById(req.params.id);
    if (!printer) {
        return res.status(404).json({ success: false, error: 'Printer not found' });
    }
...
// ========== COMMAND ROUTES ==========
// Get command history
router.get('/printers/:id/commands', asyncHandler(async (req, res) => {
    const limit = parseInt(req.query.limit) || 100;
    const commands = await PrinterModelV2.getCommandHistory(req.params.id, limit);
    res.json({ success: true, data: commands });
}));
// Save command
router.post('/printers/:id/commands', asyncHandler(async (req, res) => {
...
router.delete('/printers/:id/commands', asyncHandler(async (req, res) => {
...
router.post('/logs/batch', asyncHandler(async (req, res) => {
    ...
    const validPrinters = printerIds.length > 0
        ...
        : [];
    const validPrinterSet = new Set(validPrinters.map((p) => p.id));
...
// ========== STATS ROUTES ==========
// Get stats
router.get('/printers/:id/stats', asyncHandler(async (req, res) => {
...
// Update stats
router.put('/printers/:id/stats', asyncHandler(async (req, res) => {
...
router.post('/printers/:id/stats/increment', asyncHandler(async (req, res) => {
...
// ========== EVENTS ROUTES (NEW) ==========
// Get printer events
router.get('/printers/:id/events', asyncHandler(async (req, res) => {
    const limit = parseInt(req.query.limit) || 100;
    const events = await PrinterModelV2.getEvents(req.params.id, limit);
    res.json({ success: true, data: events });
}));
// Log event
router.post('/printers/:id/events', asyncHandler(async (req, res) => {
...
router.get('/parameter-definitions', asyncHandler(async (req, res) => {
...
router.get('/parameter-definitions/:id', asyncHandler(async (req, res) => {
...
router.get('/printers/:id/parameters', asyncHandler(async (req, res) => {
    ...
    const parameters = await ParameterModel.getAllParametersWithDefinitions(userId, req.params.id);
    res.json({ success: true, data: parameters });
}));
// Get specific parameter for a printer
router.get('/printers/:id/parameters/:parameterId', asyncHandler(async (req, res) => {
    const userId = parseInt(req.query.userId);
    if (!userId || isNaN(userId)) {
        return res.status(400).json({
            success: false,
    ...
    const parameter = await ParameterModel.getParameterWithDefinition(userId, req.params.id, req.params.parameterId);
    if (!parameter) {
...
router.put('/printers/:id/parameters/:parameterId', asyncHandler(async (req, res) => {
    ...
    await ParameterModel.setParameter(userId, req.params.id, req.params.parameterId, value);
    const updated = await ParameterModel.getParameterWithDefinition(userId, req.params.id, req.params.parameterId);
    res.json({ success: true, data: updated });
}));
// Delete parameter (revert to default)
router.delete('/printers/:id/parameters/:parameterId', asyncHandler(async (req, res) => {
    const userId = parseInt(req.query.userId);
    if (!userId || isNaN(userId)) {
        return res.status(400).json({
            success: false,
    ...
    await ParameterModel.deleteParameter(userId, req.params.id, req.params.parameterId);
    res.json({ success: true, message: 'Parameter reset to default' });
}));
// Delete all parameters for a printer (revert all to defaults)
router.delete('/printers/:id/parameters', asyncHandler(async (req, res) => {
    const userId = parseInt(req.query.userId);
    if (!userId || isNaN(userId)) {
        return res.status(400).json({
            success: false,
