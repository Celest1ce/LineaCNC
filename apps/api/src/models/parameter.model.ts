...
export interface PrinterParameterRecord {
  user_id: number;
  printer_id: string;
  parameter_id: string;
  value: any; // JSON
...
export class ParameterModel {
   ...
   */
  static async getAllDefinitions(): Promise<ParameterDefinitionRecord[]> {
   ...
   */
  static async getDefinitionById(id: string): Promise<ParameterDefinitionRecord | undefined> {
   ...
   */
  static async getDefinitionsByCategory(category: string): Promise<ParameterDefinitionRecord[]> {
    const definitions = await prisma.parameterDefinition.findMany({
      where: { category },
      orderBy: { displayOrder: 'asc' }
    });
   ...
   */
  static async getParametersByPrinter(
    userId: number,
    printerId: string
  ): Promise<PrinterParameterRecord[]> {
  ...
  static async getParameter(
    ...
    printerId: string,
    parameterId: string
  ): Promise<PrinterParameterRecord | undefined> {
   ...
   */
  static async getParameterWithDefinition(
    userId: number,
    printerId: string,
    parameterId: string
  ): Promise<any> {
   ...
   */
  static async getAllParametersWithDefinitions(
    userId: number,
    printerId: string
  ): Promise<any[]> {
    // Get all parameter definitions
    ...
    const userValueMap = new Map(
      userValues.map(v => [v.parameterId, v])
    );
   ...
   */
  static async deleteAllParameters(
    userId: number,
    printerId: string
  ): Promise<void> {
